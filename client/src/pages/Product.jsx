...
const Product = () => {
  ...
  const fetchProduct = async () => {
    try {
      const req = await fetch(`https://eggy-s-place.onrender.com/api/product/${productId}`);
      const res = await req.json();
      setProduct(res.product);

      const allProducts = await fetch(`https://eggy-s-place.onrender.com/api/product/all-products`);
      const allProductsData = await allProducts.json();
      const filteredSilarProducts = allProductsData.products.filter(
        (item) => item.category === res.product.category && item._id
      );

      setSimilarProducts(filteredSilarProducts);
    } catch (error) {
      console.log(error);
    }
  };
